...
const APP_SCREENS = {
  card:           'images/app-card.jpg',           // 매칭 카드 (홈)
  profile:        'images/app-profile.jpg',        // 상세 프로필
  tendency:       'images/app-tendency.jpg',       // 성향 분석 리포트
  inner:          'images/app-inner.jpg',          // 내면 분석 리포트
  innerAlt:       'images/app-inner-2.jpg',        // 내면 분석 리포트 2
  matchReason:    'images/app-match-reason.jpg',   // 매칭 이유
  rhythmDetail:   'images/app-rhythm-detail1.jpg', // 리듬 상세 분석 (상대)
  rhythmDetail2:  'images/app-rhythm-detail2.jpg', // 리듬 상세 분석 (회원)
  verifyMain:     'images/app-verify-1.jpg',       // 인증 항목
  verifyDetail:   'images/app-verify-2.jpg',       // 인증 항목 (필수)
  questions:      'images/app-questions.jpg',      // 주관적 선호 & 성향
  prefsMain:      'images/app-prefs-1.jpg',        // 매칭 선호도
...
const CTA_DATA = {
  // 앱 출시 후
  post: [
    { eyebrow:'Download on the', label:'App Store',     shortLabel:'App Store',  icon:'apple', href:'https://apps.apple.com/' },
    { eyebrow:'GET IT ON',        label:'Google Play',   shortLabel:'Google Play', icon:'play',  href:'https://play.google.com/' },
  ],
  // 앱 출시 전
  pre: [
    { eyebrow:'Coming soon',      label:'출시 알림 받기',   shortLabel:'알림 받기', icon:'bell', href:'#notify' },
    { eyebrow:'Limited',          label:'사전 신청하기',   shortLabel:'사전 신청',  icon:'edit', href:'#preapply' },
  ],
};
...
function App() {
  ...
  return (
    <div data-screen-label="Gyeolhada Landing">
      <Header onOpenMenu={()=>setMenuOpen(true)}/>

      {/* Mobile menu */}
      {menuOpen && (
        <div className="fixed inset-0 z-[60] bg-offwhite md:hidden flex flex-col" onClick={()=>setMenuOpen(false)}>
          <div className="h-16 px-5 flex items-center justify-between border-b border-black/5">
            <Sig.Logo height={28}/>
            <button className="w-10 h-10 grid place-items-center" aria-label="닫기">
              <svg width="18" height="18" viewBox="0 0 18 18"><path d="M3 3l12 12M15 3L3 15" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/></svg>
            </button>
          </div>
      ...
      <TweaksPanel title="Tweaks">
        ...
        <TweakRadio label="앱 출시 상태" value={t.ctaState}
          options={[{value:'post',label:'출시 후'},{value:'pre',label:'출시 전'}]}
          onChange={(v)=>setTweak('ctaState', v)}/>
        <TweakSection label="Palette"/>
        <TweakRadio label="강도" value={t.paletteIntensity}
          options={[{value:'restrained',label:'절제'},{value:'balanced',label:'균형'},{value:'rich',label:'풍부'}]}
          onChange={(v)=>setTweak('paletteIntensity', v)}/>
        <TweakSection label="Wave signature"/>
        <TweakSelect label="시그니처 스타일" value={t.waveStyle}
          options={[
            {value:'A',label:'A — 두 곡선 교차 (기본)'},
            {value:'B',label:'B — 두 곡선 교차 (굵은)'},
          ]}
          onChange={(v)=>setTweak('waveStyle', v)}/>
        <TweakSection label="Copy"/>
        <TweakSelect label="헤드라인 카피" value={t.headlineVariant}
          options={[
            {value:'experience',label:'경험 — 또 만나고 싶어집니다'},
            {value:'sharp',label:'날카로움 — 면접 같은 소개팅은 그만'},
            {value:'again',label:'또 만남 — 처음부터'},
            {value:'primary',label:'기본 — 조건보다 깊게'},
            {value:'warm',label:'따뜻한 — 천천히 그러나 깊게'},
            {value:'direct',label:'직설적 — 결이 맞아야 합니다'},
          ]}
          onChange={(v)=>setTweak('headlineVariant', v)}/>
        <TweakSection label="Typography"/>
        <TweakRadio label="한글 글꼴" value={t.fontFamily}
          options={[{value:'pretendard',label:'Pretendard'},{value:'noto',label:'Noto Sans KR'}]}
